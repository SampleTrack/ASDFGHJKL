class script:
    """
    Class to hold all text messages for the bot.
    """

    START_TEXT = (
        """
**𝒲𝑒𝓁𝒸𝑜𝓂𝑒 𝓉𝑜 𝒫𝓇𝒾𝒸𝑒 𝒯𝓇𝒶𝒸𝓀𝑒𝓇 𝐵𝑜𝓉!**

ᴇᴀꜱɪʟʏ ᴛʀᴀᴄᴋ ᴘʀɪᴄᴇꜱ ᴏꜰ ʏᴏᴜʀ ꜰᴀᴠᴏʀɪᴛᴇ ᴘʀᴏᴅᴜᴄᴛꜱ ꜰʀᴏᴍ ᴀᴍᴀᴢᴏɴ 🛒 ᴀɴᴅ ꜰʟɪᴘᴋᴀʀᴛ 📦.

**--🔍 ᴊᴜꜱᴛ ꜱᴇɴᴅ ᴍᴇ ᴀ ᴘʀᴏᴅᴜᴄᴛ ʟɪɴᴋ, ᴀɴᴅ ɪ’ʟʟ:--**

📉 ᴛʀᴀᴄᴋ ᴘʀɪᴄᴇ ᴄʜᴀɴɢᴇꜱ ᴀᴜᴛᴏᴍᴀᴛɪᴄᴀʟʟʏ  
🔔 ɴᴏᴛɪꜰʏ ʏᴏᴜ ᴡʜᴇɴ ᴛʜᴇ ᴘʀɪᴄᴇ ᴅʀᴏᴘꜱ  
📊 ꜱʜᴏᴡ ᴘʀᴏᴅᴜᴄᴛ ᴅᴇᴛᴀɪʟꜱ & ᴄᴜʀʀᴇɴᴛ ᴘʀɪᴄᴇ  
✨ ꜱᴛᴀʏ ᴀʜᴇᴀᴅ ᴏꜰ ᴅᴇᴀʟꜱ ᴀɴᴅ ɴᴇᴠᴇʀ ᴍɪꜱꜱ ᴀ ᴅɪꜱᴄᴏᴜɴᴛ!
"""
    )

    HELP_TEXT = (
        """
**--ℹ️ 𝙷𝚘𝚠 𝚝𝚘 𝚄𝚜𝚎 𝙿𝚛𝚒𝚌𝚎 𝚃𝚛𝚊𝚌𝚔𝚎𝚛 𝙱𝚘𝚝⁉️--**

**1️⃣ Start Tracking**

🇸🇪🇳🇩 ᴍᴇ ᴀɴʏ ᴀᴍᴀᴢᴏɴ 🛒 ᴏʀ ꜰʟɪᴘᴋᴀʀᴛ 📦 ᴘʀᴏᴅᴜᴄᴛ ʟɪɴᴋ.  
🇨ʟɪᴄᴋ **Start Tracking** ᴛᴏ ʙᴇɢɪɴ ᴍᴏɴɪᴛᴏʀɪɴɢ ᴛʜᴇ ᴘʀɪᴄᴇ.

**2️⃣ View Your Trackings**

Use **/my_trackings** ᴛᴏ ꜱᴇᴇ ᴀʟʟ ᴘʀᴏᴅᴜᴄᴛꜱ ʏᴏᴜ’ʀᴇ ᴛʀᴀᴄᴋɪɴɢ.

**3️⃣ Remove Tracking**

ʀᴇᴍᴏᴠᴇ ᴀɴʏ ᴘʀᴏᴅᴜᴄᴛꜱ ʏᴏᴜ ɴᴏ ʟᴏɴɢᴇʀ ᴡᴀɴᴛ ᴛᴏ ᴛʀᴀᴄᴋ.

**📢 Support & Issues**

𝒮𝓉𝒶𝓎 𝓊𝓅𝒹𝒶𝓉𝑒𝒹, 𝓈𝒶𝓋𝑒 𝓂𝑜𝓃𝑒𝓎 💰, 𝒶𝓃𝒹 𝓃𝑒𝓋𝑒𝓇 𝓂𝒾𝓈𝓈 𝒶 𝒹𝑒𝒶𝓁! 🚀
"""
    )

    ABOUT_TEXT = (
        "**ℹ️ About Me**\n\n"
        "🤖 **Name:** My Bot\n"
        "👨‍💻 **Dev:** [Developer](https://t.me/YourUsername)\n"
        "🐍 **Language:** Python 3\n"
        "🔥 **Library:** Pyrogram"
    )

    STATS_TEXT = (
        "🍀 **Bot Statistics** 🍀\n\n"
        "🙋‍♂️ **Total Users:** `{}`"
    )
    LOG_MESSAGE = (
        f"#New User\n"
        f"User ID: `{user_id}`\n"
        f"Username: {full_name}{username}\n"
        f"Chat type: `{chat_type}`\n"
        f"Date (IST): `{date}`\n"
        f"Time (IST): `{time}`\n"
    )
